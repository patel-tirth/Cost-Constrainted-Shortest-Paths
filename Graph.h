...
class graph {
  ...
  private:
    ...
    //   double cost;
    //   double time;
    //   signature ( int vtx_id=0,double _weight=1.0, double _wt2=1.0) 
  ...
  struct myQ{
    ...
    const noexcept
    {
      const double t1 = get<1>(tpl1);
      const double t2 = get<1>(tpl2);
      const double c1 = get<0>(tpl1);
      const double c2 = get<0>(tpl2);
     ...
     while (pq.empty() == false)
    {
        ...
        //  cout<< get<0>(t) <<" "<< get<1>(t) << " "<< get<2>(t) <<endl;
            ...
            for(edge &e : vertices[i].outgoing) {  // for each outgoing edge for vertex 
                    ...
                    double cost2 = e.weight;  
                    double time2 = e.weight2;
                    string v =  id2name(e.vertex_id);
